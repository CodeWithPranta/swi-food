   ...
   public function mount()
{
    ...
    if ($user->user_type == 2) {
    // 1️⃣ Users who ordered from this vendor
    $customerIds = $user->vendorApplication
        ? Order::where('vendor_application_id', $user->vendorApplication->id)
            ->pluck('user_id')
            ->unique()
        : collect();
    ...
    $customers = \App\Models\User::whereIn('id', $customerIds)
        ->with([
            'sentMessages' => fn($q) => $q->where('receiver_id', $user->id)->latest(),
            'receivedMessages' => fn($q) => $q->where('sender_id', $user->id)->latest(),
        ])
        ->get();

    // 2️⃣ Vendors this user has ordered from
    $vendorIds = Order::where('user_id', $user->id)
        ->pluck('vendor_application_id')
        ->unique();

    $vendors = \App\Models\VendorApplication::with('user')->whereIn('id', $vendorIds)->get()
        ->pluck('user')
        ->filter(); // remove nulls

    // 3️⃣ Merge customers + vendors as User models
    $this->homerestaurants = $customers->merge($vendors)
        ->unique('id')
        ->sortByDesc(fn($contact) => max(
            $contact->sentMessages->first()?->created_at?->timestamp ?? 0,
            $contact->receivedMessages->first()?->created_at?->timestamp ?? 0
        ))
        ->values();
